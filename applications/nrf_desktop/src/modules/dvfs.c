...
static enum state module_state = STATE_INITIALIZING;
...
static void handle_dvfs_error(enum dvfs_frequency_setting target_freq, int32_t err)
{
	if (k_work_delayable_is_pending(&dvfs_retry_data.dvfs_retry_work)) {
		return;
	}
...
static void set_dvfs_freq(enum dvfs_frequency_setting target_freq)
{
	...
	if (ret) {
		handle_dvfs_error(target_freq, ret);
	} else {
