...
The nRF Peripheral Sensor node is an application that periodically acquires data from a Serial Peripheral Interface (SPI) and Two-wire Interface (TWI) devices, advertises, and enables the reading of sensor values from Bluetooth® service in a power-efficient way.
...
The application periodically performs burst measurement of the environmental and IMU sensors in specified intervals and does additional burst measurement when triggered by the interrupt from the ``ADXL362`` sensor.
...
``LED 1`` lit when the application is active
...
CONFIG_APP_CONTROL_BURST_INTERVAL_S
   ...
   If sensor data burst is not triggered by :c:struct:`wake_up_event` from IMU, burst is performed every interval value.
...
#. Run the following command to build the application code:

   .. code-block:: console

      west build -b nrf54l15pdk_nrf54l15_cpuapp -- -DSHIELD=pca63565

   or use twister test case:

   .. code-block:: console

      west build -b nrf54l15pdk_nrf54l15_cpuapp -T applications.peripheral_sensor_node .

#. Program the application to the kit:

   .. code-block:: console

      west flash

If you use the |nRFVSC|, specify ``-DSHIELD=pca63565`` in the **Extra Cmake arguments** field when `setting up a build configuration <How to work with build configurations_>`_.
...
#. To observe the logs, connect to the development kit with a terminal emulator.
