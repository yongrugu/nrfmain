...
* Code that runs on the internal flash.
* Code that runs on a supported external flash over Quad Serial Peripheral Interface (QSPI) using Execute in place (XIP).
...
Make sure you meet the following requirements to make use of this feature:

* Board based on nRF52840, nRF5340, or nRF91 Series SoCs, compatible with :ref:`Hardware model v2 (HWMv2) <hwmv1_to_v2_migration>`
...
.. caution::
    At present, MCUboot images cannot be linked together.
    This means that both parts of the firmware update must be loaded properly before an update is initiated.
    If one part fails to update, the image on the device will be unbootable and will likely fault.

Network core image updates
**************************

The QSPI XIP split image feature can be used and will retain support for network core image updates on nRF5340 devices if MCUboot is configured for the Swap using move mode or the Upgrade only mode.
...
Set additional Kconfig options in the application's :file:`prj.conf` to allow it to execute code from XIP:
...
There is no in-built check for this setting, so the priority must be set manually based upon the code that has been relocated.
For example, if you relocate the shell subsystem, the initialization priority of any of that subsystem must be higher than the priority of QSPI, and the value of :kconfig:option:`CONFIG_SHELL_BACKEND_SERIAL_INIT_PRIORITY` must be greater than :kconfig:option:`CONFIG_NORDIC_QSPI_NOR_INIT_PRIORITY`.
...
Here's an example for the Thingy:53, which only supports DSPI and PP:
...
The following table lists files that are generated when building a QSPI XIP split image application.
