...
{
	/* Used to determine if the clock sync is required. */
	static bool first_state_change_after_bootup = true;
	...
	if (fp_account_key_present != provisioned) {
		...
		first_state_change_after_bootup = false;
		return;
	}
	...
	app_fp_adv_request(&fp_adv_trigger_clock_sync,
			   fmdn_provisioned && first_state_change_after_bootup);
	app_fp_adv_request(&fp_adv_trigger_fmdn_provisioning, false);
