				  ...
				  struct nct_dc_endpoints *const eps)
{
	...
	if (endpoint_obj != NULL) {
		err = json_decode_and_alloc(endpoint_obj, &eps->base);
		if (err) {
			return err;
		}
	}
	...
	err = json_decode_and_alloc(tx_obj, &eps->tx);
	if (err) {
	 ...
	 */
	len_tmp = eps->tx.size + sizeof(NRF_CLOUD_BULK_MSG_TOPIC);
	eps->bulk.utf8 = (uint8_t *)nrf_cloud_calloc(len_tmp, 1);
	if (eps->bulk.utf8 == NULL) {
		LOG_ERR("Could not allocate memory for bulk topic");
		return -ENOMEM;
	}

	eps->bulk.size = snprintk((char *)eps->bulk.utf8, len_tmp, "%s%s",
				  (char *)eps->tx.utf8, NRF_CLOUD_BULK_MSG_TOPIC);

	/* Populate bin endpoint topic by copying and appending /bin to the parsed
	 * tx endpoint (d2c) topic.
	 */
	len_tmp = eps->tx.size + sizeof(NRF_CLOUD_JSON_VAL_TOPIC_BIN);
	eps->bin.utf8 = (uint8_t *)nrf_cloud_calloc(len_tmp, 1);
	if (eps->bin.utf8 == NULL) {
		LOG_ERR("Could not allocate memory for bin topic");
		return -ENOMEM;
	}

	eps->bin.size = snprintk((char *)eps->bin.utf8, len_tmp, "%s%s",
				 (char *)eps->tx.utf8, NRF_CLOUD_JSON_VAL_TOPIC_BIN);

	err = json_decode_and_alloc(json_object_decode(topic_obj,
		NRF_CLOUD_JSON_KEY_CLOUD_TO_DEVICE), &eps->rx);
	if (err) {
